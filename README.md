...
In the above workflow, i have actually used a [Firecrawl](https://www.firecrawl.dev/) as my MCP server, the other alternatives are [Brave Browser's Search API](https://brave.com/search/api/) which works the same. Every MCP server has there respected API key, so put it as per. 
